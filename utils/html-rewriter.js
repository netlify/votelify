(function(stream_1){'use strict';function __export(m) {
...
exports.RssHandler = FeedHandler_1.FeedHandler;Object.defineProperty(exports, "__esModule", { value: true });
...
exports.AllHtmlEntities = html5_entities_1.Html5Entities;var __createBinding = (undefined && undefined.__createBinding) || (Object.create ? (function(o, m, k, k2) {
...
Object.defineProperty(exports, "stringify", { enumerable: true, get: function () { return stringify_1.default; } });var css_what_1=/*#__PURE__*/Object.freeze({__proto__:null});Object.defineProperty(exports, "__esModule", {
  value: true
});
exports.HTMLRewriter = exports.TextChunk = exports.Element = exports.ContentType = void 0;
const selfClosingTags = new Set(["area", "base", "basefont", "br", "col", "command", "embed", "frame", "hr", "img", "input", "isindex", "keygen", "link", "meta", "param", "source", "track", "wbr"]);
var ContentType;
...
class Element {
  ...
  serializeStart(out) {
    ...
    if (this.replaceIns != null) {
      this.replaceIns.serialize(out); // cleanup

      this.wasReplaced = true;
      this.noChildNodes = true;
      this.replaceIns = null; // definitely never read
    ...
    const attrs = Object.entries(this.attributes).map(([key, val]) => val != null ? ` ${key}="${val}"` : ` ${key}`).join("");
  ...
  matches(query) {
    if (typeof query === "string") {
      const queries = undefined(query);
      return this.matchesAny(queries);
    }

    let elem = this;
    let matchUpwards = false; // stack of selectors to test against an element

    const selectorStack = []; // matches from right to left
...
class HTMLRewriterTransformer extends stream_1.Writable {
  constructor(handler, out) {
    ...
    this.parser = null;
    this.stack = [];
    this.stream = new undefined(this);
  }
  ...
  ontext(text) {
    ...
    if (current && (current.noChildNodes || current instanceof DiscardedElement)) {
      return;
    }
...
class HTMLRewriter {
  ...
  on(selector, cb) {
    const queries = undefined(selector);
    this.rules.push({
      queries,
      cb
    });
...
exports.HTMLRewriter = HTMLRewriter;}(stream_1));